...
class Gardient:
    def __init__(self, start: list, eps):
        ...
        self.diffx, self.diffy = diff(self.x ** 2 + (self.y - 3) ** 2, self.x), \
                                diff(self.x ** 2 + (self.y - 3) ** 2,self.y)  #здесь находим производную от функции
        self.__x1,self.__x2 = start[0], start[1]
        self.__startX1, self.__startX2 = antigardient(self.diffx, self.x, self.__x1), \
                                         antigardient(self.diffy, self.y, self.__x2)
        self.__eps = eps
        self.__k = 0
    ...
    def algoritm(self):   #метод наискорейшего спуска
        if sqrt(self.__startX2 ** 2 + self.__startX1 ** 2) < self.__eps:
            ...
            def func(x):
                ...
                return 2 * (self.__x1 + self.__startX1 * x)**2 + (self.__x2 + self.__startX2 * x - 3) ** 2
